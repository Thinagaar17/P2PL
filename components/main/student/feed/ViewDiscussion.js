...
function ViewDiscussion(props) {
  ...
  const sendReport = (rid) => {
    ...
    const alert = () => {
      ...
      Alert.alert('Done', 'Your report has been sent and will be reviewed', [
    ...
    if (userPosts.userId !== userId) {
      ...
      fetch('https://exp.host/--/api/v2/push/send', {
        ...
        body: JSON.stringify({
          ...
          priority: 'normal',
          data: { id: discussionId, description: newComment, userId: userId, ownerId: ownerId },
        }),
      });
    }
    //  ------------------ Sending Push Notification To Author of Discussion -----------------------
