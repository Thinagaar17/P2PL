...
function Reply(props) {
      ...
      if (mainCommentOwner.status !== 1) {
        firebase
          ...
          .add({
            ...
            mainId: mainCommentUserId,
            mainCommentAuthorName: mainCommentAuthorName,
            pushToken: pushToken
          });
        ...
        fetch('https://exp.host/--/api/v2/push/send', {
          ...
          body: JSON.stringify({
            ...
            priority: 'normal',
            data: { cid: mainCommentId, description: newReply, userId: userId, discussionId: discussionId, mainId: mainCommentUserId, mainCommentAuthorName: mainCommentAuthorName,  pushToken: pushToken},
          }),
        });
      }
    }
  ...
  const UploadCommentV2 = () => {
    ...
    const subCommentOwner = allUsers.find(el => el.id === repliedSubCommentUserId);
    //  ------------------ Sending Push Notification To Author of Reply Comment -----------------------
    if (repliedSubCommentUserId !== userId) {
      if (subCommentOwner.status !== 1) {
        firebase
          .firestore()
          .collection('users')
  ...
  const addLike = (nol, y, z) => {
    ...
    const mainCommentOwner = allUsers.find(el => el.id === mainComment.userId);
    if (mainCommentUserId !== userId) {
      if (mainCommentOwner.status !== 1) {
        firebase
          .firestore()
  ...
  const AddLikeToReplyComment = (rcid, nol, lb, uid) => {
    ...
    const subCommentOwner = allUsers.find(el => el.id === uid);
    console.log(pushToken);
    firebase
      .firestore()
      .collection('Comment')
